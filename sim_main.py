...
parser.add_argument("--action_source", type=str, default="dds", 
                   choices=["dds", "file", "trajectory", "policy", "replay", "dds_wholebody", "auto"], 
                   help="Action source")
        ...
        with contextlib.suppress(KeyboardInterrupt), torch.inference_mode():
            while simulation_app.is_running() and controller.is_running:
                ...
                try:
                    ...
                    err_msg = str(step_e)
                    # Catch physics tensor invalidation error - occurs when object is reset/deleted
                    # This is a known issue with RTX 50-series + Isaac Sim 5.1
                    if "invalidated" in err_msg or "DOF" in err_msg or "simulation view" in err_msg.lower():
                        print(f"\n[Warning] Physics tensor invalidated (likely object reset). Recovering... ({step_e})")
                        try:
                            env.sim.reset()
                            env.reset()
                            print("[Recovery] Environment reset successfully, continuing simulation.")
                        except Exception as recover_e:
                            print(f"[Recovery] Failed to recover environment: {recover_e}")
                            raise step_e
                    else:
                        raise step_e
                ...
                if current_time - last_stats_time >= args_cli.stats_interval:
                    ...
                    # ---- TRÍCH XUẤT TỌA ĐỘ 3D VẬT THỂ ----
                    try:
                        print(f"--- 3D Coordinates (Ground Truth) ---")
                        if hasattr(env, "scene"):
                            # Lấy tọa độ gốc robot
                            try:
                                r_pos = env.scene["robot"].data.root_pos_w[0].cpu().numpy()
                                print(f"[Robot]        x: {r_pos[0]:.4f}, y: {r_pos[1]:.4f}, z: {r_pos[2]:.4f}")
                            except Exception:
                                pass
                            
                            # Lấy tọa độ khối trụ
                            try:
                                o_pos = env.scene["object"].data.root_pos_w[0].cpu().numpy()
                                print(f"[Cylinder]     x: {o_pos[0]:.4f}, y: {o_pos[1]:.4f}, z: {o_pos[2]:.4f}")
                            except Exception:
                                pass
                                
                            # (Bỏ qua iteration vì InteractiveScene không có .items() trong phiên bản này)
                    except Exception as e:
