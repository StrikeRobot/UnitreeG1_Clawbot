...
class TableCylinderSceneCfg(InteractiveSceneCfg): # inherit from the interactive scene configuration class
    ...
    # Target Basket / Tray (rổ chứa)
    basket = AssetBaseCfg(
        prim_path="/World/envs/env_.*/Basket",
        init_state=AssetBaseCfg.InitialStateCfg(pos=[0.15, 0.45, 0.825], # Đặt bên cạnh tay phải, trên mặt bàn (z~0.82)
                                                rot=[1, 0, 0, 0]),
        spawn=sim_utils.CuboidCfg(
            size=(0.15, 0.15, 0.02), # Cái khay vuông 15x15cm, dày 2cm
            rigid_props=sim_utils.RigidBodyPropertiesCfg(kinematic_enabled=True),
            collision_props=sim_utils.CollisionPropertiesCfg(),
            visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(1.0, 0.5, 0.0), metallic=0.1) # Màu cam (Orange)
        )
    )
    # Ground plane
