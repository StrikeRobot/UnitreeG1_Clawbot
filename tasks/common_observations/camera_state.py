...
def get_camera_image(
    ...
    if "front_camera" in camera_keys:
        ...
        # extract depth 
        depth_tensor = None
        pd = env.scene["front_camera"].data.output
        output_keys = list(pd.keys())
        with open("/tmp/camera_keys.txt", "w") as f:
            f.write(f"Camera output keys: {output_keys}\n")
            if "distance_to_image_plane" in output_keys:
                f.write(f"distance shape: {pd['distance_to_image_plane'][0].shape}\n")
        
        if "distance_to_image_plane" in env.scene["front_camera"].data.output:
            depth_tensor = env.scene["front_camera"].data.output["distance_to_image_plane"][0]
        elif "distance_to_camera" in env.scene["front_camera"].data.output:
            depth_tensor = env.scene["front_camera"].data.output["distance_to_camera"][0]
        elif "depth" in env.scene["front_camera"].data.output:
            depth_tensor = env.scene["front_camera"].data.output["depth"][0]
        ...
        if depth_tensor is not None:
            ...
            # format to HxWx4 uint8 memory map to bypass MultiImageWriter restriction
            import numpy as np
            depth_np = depth_np.astype(np.float32)
            if depth_np.ndim == 2:
                depth_np = np.expand_dims(depth_np, axis=-1)
            # depth_np is now (H, W, 1) float32
            depth_bytes_img = depth_np.view(np.uint8).reshape(depth_np.shape[0], depth_np.shape[1], 4)
            images["head_depth"] = depth_bytes_img
        else:
            pass # No depth found in the camera data
