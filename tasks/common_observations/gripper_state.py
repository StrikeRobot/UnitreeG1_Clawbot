...
def get_robot_gipper_joint_states(
    ...
    if _obs_cache["device"] != device or _obs_cache["gripper_idx_t"] is None:
        try:
            ...
            # index 0 MUST be right, index 1 MUST be left (as assumed in DDS publish code below)
            gripper_joint_indices = [
                all_joint_names.index("right_hand_Joint1_1"),
                all_joint_names.index("left_hand_Joint1_1")
            ]
        except (ValueError, AttributeError):
            gripper_joint_indices = [31, 29] # fallback
